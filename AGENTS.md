...
- 0.4.2 启动性能优化：桌面浏览不得提前导入 session、OpenCV 或 OCR；任务依赖在工作线程内加载。ttk 九宫格中心会被平铺，保持足够大的中心图块且显式取消底图最小尺寸；不以缩小底图节省内存。图卡依据 Canvas 坐标判断可见性，滚动补画并释放销毁控件的事件绑定。修改时运行图鉴可见冒烟，检查末尾图片、键盘焦点和返回页面的滚动位置。交付状态见 status.md。
