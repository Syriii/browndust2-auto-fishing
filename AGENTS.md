...
- 本目录是 GitHub 仓库根目录。`main.py` 为启动入口，`scripts/build_release.py` 为构建入口，业务代码与 OCR、识别模板位于 `src/bd2_fishing/`。
...
- `.local/config.ini` 是个人源码配置；唯一默认值在 `src/bd2_fishing/resources/default.ini`，由 settings 通过包资源读取。修改默认值不覆盖已有个人配置；部署保留其他配置和注释。
- `.venv/`、`.local/`、缓存和 egg-info 是本地数据，常规源码搜索应排除。真实回归图保存在 `tests/fixtures/`，运行时模板在 `src/bd2_fishing/game/fishing/assets/`。
...
- 使用 Ruff 统一检查和格式，规则在 pyproject。运行 `python -m ruff check src scripts tests main.py setup.py`、`python -m ruff format --check src scripts tests main.py setup.py` 及 `python scripts/checks/check_architecture.py`；修改依赖边界须更新规范和回归，不能用局部忽略绕过。
...
- 游戏输入统一经过 `src/bd2_fishing/infrastructure/windows/input.py`；工作线程等待使用 `run_control.sleep`，长循环设置取消检查，不绕过取消锁。
...
- 显示器选择与相机创建使用同次枚举的原生设备/输出对象，不能把实时索引交给 DXcam 旧工厂缓存。依赖固定 DXcam 0.3.0，对象适配位于 `src/bd2_fishing/infrastructure/windows/display.py`，升级须核对接口。
...
仅任务要求发布或覆盖时部署。使用 `scripts/build_release.py` 的 `.local/releases/BD2_AutoFishing/` 成套产物，保留 Tk/Tcl、OCR 模型和运行库。覆盖前确认程序退出，备份旧 EXE、`_internal/` 和配置，保留用户配置及日志，核对部署与构建产物哈希。递归移动/删除前验证解析后的绝对路径在预期目标内，使用 PowerShell 原生文件命令。
