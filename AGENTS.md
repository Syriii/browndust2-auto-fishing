...
- 下拉框统一使用 theme 的状态配色；覆盖 clam 的 readonly+focus 白字时须同步前景与背景。箭头图元保持自然尺寸，不纵向平铺；保留原生菜单、键盘及禁用行为。
- 主窗口只保留系统标题栏及原生图标，不再添加重复的应用名称横幅；页面标题保留。布局改动核对默认、最小窗口与短屏缩放。
- 0.4.6 本地测试版增加目标时段策略：app.target_time_policy 默认为 wait，continuous 是用户明确选择的不等待昼夜模式；启动时固定到 collection.wait_for_time，工作线程不读 Tk 变量。持续模式仍核对钓场、导航、待机、完成与取消，不绕过其他输入边界。日志旁鱼获按 collection.run_id 查询，停止后保留、下一次 begin 重置，最多绘制最近 30 条；展示不能修改历史或抵扣待办。回归含 smoke_session_catches.py，交付见 status.md。

- 2026-09-15 目标启动回归：普通启动读到岛名但未确认待机时交给 prepare_start，不按换岛失败退出，也不能凭岛名直接抛竿；已进入导航或显式换岛仍需原有确认。目标等待日志须区分已确认的白天／夜晚与读数未知，报告目标名，重复等待不逐次刷屏。证据见 docs/development/cases/2026-09-15-target-start.md。

- 0.4.5 修复连续选鱼与切页闪烁：选中状态不进入网格重建键，详情控件与图片引用保留；鱼获按分类、日期、视图、列数和 journal.revision 决定是否重新读取／绘制。目标内容未变时复用。所有页面在旧页下方准备后切换，不能先显示再重建。修改时运行 `smoke_gallery_refresh.py`，覆盖全 84 图、网格／列表、连续选鱼、回页、缓存淘汰和鱼获首次绘制；新增鱼获必须随 revision 刷新。交付状态见 status.md。

- 0.4.4 按用户要求恢复钓鱼页常驻日志，取消展开／收起入口；任务设置与本次进度并排。图鉴与鱼获改为左侧密集小图／列表、右侧固定详情，切换视图不移动详情区。鱼获只显示统一图鉴图片；旧记录名称与已核实图标的资料关联不能补写确认状态或完成目标。布局检查覆盖最小窗口、125% 短屏、日志高度和控件边界；日志队列与文件保存规则保持原实现。交付状态见 status.md。

- 0.4.3 修复图鉴首次白屏：切页先在旧页下方完成 `CataloguePage.prepare()`，显式绘制可见 Canvas 鱼卡并完成列数调整，再抬到最上层。遮挡状态不保证收到 Expose，不得只依赖空闲回调完成首屏。可见冒烟须检查读图时旧页仍在上层、显示前首屏图片齐全；保留按需加载。交付状态见 status.md。

- 0.4.2 已发布并核验本地与官方 EXE，证据见 `docs/development/cases/2026-09-14-release-0.4.2.md`。启动性能优化：桌面浏览不得提前导入 session、OpenCV 或 OCR；任务依赖在工作线程内加载。ttk 九宫格中心会被平铺，保持足够大的中心图块且显式取消底图最小尺寸；不以缩小底图节省内存。图卡依据 Canvas 坐标判断可见性，滚动补画并释放销毁控件的事件绑定。修改时运行图鉴可见冒烟，检查末尾图片、键盘焦点和返回页面的滚动位置。交付状态见 status.md。
...
- 用户已认可 `docs/design/ui-workspace-proposal.md` 第四版并授权推进实现；目标鱼多选、全鱼种记录和最大／最小条件已接入 0.4.0 源码，EXE 交付状态以 status.md 顶部及验收记录为准。图鉴滚轮连续浏览，网格／列表用图标切换且不改变窗口高度；按用户 2026-09-15 的后续要求，网格和列表都使用右侧固定详情，字段一致，不显示繁简别名行或通用机制占位。浏览不自动改钓场、创建任务或出售／锁定物品。
...
- 等待时段模式读取游戏时钟图标，两次当前帧确认，不按现实时间或夜景背景推断；缺失或矛盾时等待并保持取消及窗口保护。用户选择 continuous 后明确跳过昼夜筛选，继续在目标钓场钓鱼。
...
鱼获历史按捕获时间倒序和本地日期分组，日期筛选与成就分类同时生效；不能按插入顺序代替捕获时间，异常时间单列未确认，浏览和筛选不能修改记录。
