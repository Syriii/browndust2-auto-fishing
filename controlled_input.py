...
import win32api
import run_control
from display_support import normalize_virtual_point
...
def _move_virtual(x, y):
    ...
    payload.mi = _input.MouseInput(nx, ny, 0, 0x0001 | 0x8000 | 0x4000,
...
def moveTo(x=None, y=None, duration=None, tween=None, logScreenshot=False, _pause=True, relative=False):
    if relative:
        return run_control.call_input(_input.moveTo, x, y, duration=duration, tween=tween,
                                      logScreenshot=logScreenshot, _pause=_pause, relative=True)
