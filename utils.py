...
class DxCameraCapture:
    ...
    def _select_camera(self, region):
        from display_support import create_camera_for_region
        ...
        logging.getLogger(__name__).info(
            ...
            output.name, output.device_idx, output.output_idx, output.bounds)
...
class WindowGuard:
    ...
    def __call__(self):
        if self.require_foreground:
            from session_support import input_desktop_available
...
[backpack]
; true：识别到满包后自动出售；false：满包时停止，等待手动整理
...
[app]
; 图形界面启动时使用的钓场；也可在界面选择
location = 自动识别
; 仅任务运行期间防止系统自动息屏/睡眠，不阻止锁屏或主动睡眠
prevent_sleep = false

[diagnostics]
; true 时逐帧 QTE 数据写入滚动日志文件；排查后建议关闭
qte_detail_log = false
; 记录 QTE 反馈、失败/未确认截图及整条鱼结算证据；可在页面关闭
; 只观察游戏结果，不改变 QTE 按键策略
qte_feedback_enabled = true
; 上钩超时时保存峰值截图、颜色遮罩、现场截图和参数，后台写盘
enabled = true
; 最多每 60 秒保存一份，循环覆盖 10 个诊断包
interval_seconds = 60
max_events = 10
"""
