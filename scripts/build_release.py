...
def main() -> None:
    ...
    try:
        ...
        import rapidocr  # noqa: F401 -- verify native dependencies actually import
    ...
    for required in ("init.tcl", "tk.tcl", "_tkinter.pyd"):
        if not any(package_dir.rglob(required)):
            raise SystemExit(
                f"Required GUI runtime missing: {required}; check Tcl/Tk access before packaging"
            )
