...
def main():
    ...
    parser.add_argument("--hook-upper-hue", type=int, choices=range(181), default=None,
                        help="仅本次实测覆盖上钩色相上限，不修改 config.ini")
    args = parser.parse_args()
    if args.seconds <= 0:
        parser.error("--seconds must be positive")

    fishing.utils.setup_logging()
    region = fishing.utils.get_window_region(fishing.GAME_TITLE)
    if region is None:
        raise SystemExit("未找到游戏窗口")
    hwnd = win32gui.FindWindow(None, fishing.GAME_TITLE)
    print(f">>> 实测窗口: {region.as_tuple()}", flush=True)
    config = fishing.utils.read_ini()
    ...
    context = fishing.ocr_utils.build_ocr_context(config, region)
    bot = fishing.FishingBot(config, region, context)
    ...
    originals = {name: getattr(fishing.pydirectinput, name) for name in action_names}
    ...
    try:
        ...
        for name, action in originals.items():
            setattr(fishing.pydirectinput, name, guarded(action))
        bot.run()
        ...
        for name, action in originals.items():
            setattr(fishing.pydirectinput, name, action)
