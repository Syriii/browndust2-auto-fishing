...
class Recorder:
        ...
        try:
            with ZipFile(self.output / "frames.zip", "w", compression=ZIP_STORED) as archive:
                while not self.done.is_set() or not self.queue.empty():
                    try:
                        frame, t, phase = self.queue.get(timeout=.05)
                    except queue.Empty:
                        continue
                    ...
                    self.frames.append(dict(file=name, t=t, phase=phase,
    ...
    def close(self):
        ...
        metadata = dict(events=self.events, frames=self.frames, dropped=self.dropped,
...
class SharedCapture:
    ...
    def capture(self):
        ...
        try:
            ...
            with run_control.use_control(self.recorder.control), run_control.use_input_guard(
                utils.WindowGuard(fishing.GAME_TITLE, self.region, require_foreground=True)
            ), utils.DxCameraCapture(window_region=self.region) as camera:
                saved_at = float("-inf")
                while not self.stop.is_set():
                    ...
                    if frame is not None:
                        ...
                        with self.lock:
                            self.frame = frame
                            self.sequence += 1
                        interval = 1/20 if self.recorder.control.phase == "QTE 进行中" else .25
                        if now-saved_at >= interval:
                            self.recorder.offer(frame, now)
                            saved_at = now
                    run_control.sleep(.01)
        except BaseException as exc:
            self.error = exc
    ...
    def grab(self, region):
        ...
        if self.error:
            raise self.error
        region = region if isinstance(region, utils.Rect) else utils.Rect(*region)
        with self.lock:
...
def load_debug_config(path=None):
    """工作树可显式读取原目录配置；没有部署配置时使用本工作树源码配置。"""
    if path is None:
        deployed = ROOT.parent / "config.ini"
        path = deployed if deployed.is_file() else ROOT / "config.ini"
...
def main():
    ...
    parser.add_argument("--config", type=Path, help="只读指定配置文件；工作树中可指向原目录的用户配置")
    parser.add_argument("--stop-file", type=Path, help="测试时创建此文件可请求正常停止并完成截图写盘")
    parser.add_argument("--location", choices=[v.value for v in FishingLocation], default="亚特兰蒂斯")
    parser.add_argument("--probe-outcomes", action="store_true",
                        help="仅调试：分别在色条外和蓝条内按一次，采集失败与普通命中反馈")
    parser.add_argument("--probe-escape", action="store_true",
                        help="仅调试：进入 QTE 后只观察、不按键，采集超时结束证据")
    parser.add_argument("--feedback", action="store_true", help="启用新增的只读 QTE 结果观察和失败留图")
    ...
    output = ROOT / "debug" / ("qte_live_"+time.strftime("%Y%m%d_%H%M%S"))
    output.mkdir(parents=True)
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s %(levelname)s %(name)s: %(message)s",
                        handlers=[logging.StreamHandler(), logging.FileHandler(output/"session.log", encoding="utf-8")])
    logging.getLogger().handlers[0].setLevel(logging.INFO)
    logging.getLogger("diagnostic_logging").setLevel(logging.DEBUG)
                ...
                for target in ("outside", "blue"):
                    ...
                    while time.monotonic() < deadline:
                        ...
                        if cursor is not None:
                            ...
                            if not on_yellow and ((target == "outside" and not on_blue) or
                                                  (target == "blue" and on_blue)):
                                recorder.event("probe_target", target=target, cursor=cursor)
                                self._press_qte()
                                run_control.sleep(1)
                                break
                        self._sleep_loop()
        fishing.QTE_STRATEGIES_MAP[FishingLocation(args.location)] = ProbeStrategy
    if args.probe_escape:
        ...
        fishing.QTE_STRATEGIES_MAP[FishingLocation(args.location)] = ObserveEscapeStrategy
    capture_factory, original_press = fishing.DxCameraCapture, controlled_input.press
    ...
    if not args.no_full_frames:
        fishing.DxCameraCapture = lambda **kwargs: SharedCapture(recorder, **kwargs)
    ...
    timer.start()
    def watch_stop_file():
        while not control.stopped.wait(.1):
    ...
    try:
        with run_control.use_control(control):
            fishing.run_once(config, location=FishingLocation(args.location), interactive=False)
        ...
        controlled_input.press, fishing.DxCameraCapture = original_press, capture_factory
        recorder.close()
