...
from diagnostic_logging import QTETrace, ocr_log_context, route_rapidocr_logs, trace_qte
from ocr.ocr_service import get_result_from_ocr
import run_control
...
class OCRLoggingTests(unittest.TestCase):
    ...
    def test_optional_empty_and_required_empty_keep_different_severity(self):
        ...
        for optional, purpose, level in ((True, "背包满提示", logging.INFO), (False, "钓场地点", logging.WARNING)):
            with self.subTest(optional=optional), self.assertLogs("ocr.ocr_service", level="INFO") as logs:
                self.assertEqual(get_result_from_ocr(capture, engine, Rect(0, 0, 8, 4),
                                 purpose=purpose, expected_empty=optional), [])
    ...
    def test_no_new_frame_is_not_reported_as_ocr_failure(self):
        engine = Mock()
        with self.assertLogs("ocr.ocr_service", level="INFO") as logs:
            self.assertIsNone(get_result_from_ocr(Mock(grab=Mock(return_value=None)), engine, Rect(0, 0, 8, 4)))
    ...
    def test_ocr_exception_retains_traceback_and_capture_error_still_propagates(self):
        ...
        with self.assertLogs("ocr.ocr_service", level="ERROR") as logs:
        ...
        with self.assertLogs("ocr.ocr_service", level="ERROR"), self.assertRaises(OSError):
...
class QTELoggingTests(unittest.TestCase):
    def test_both_real_strategies_keep_press_and_end_behavior(self):
        ...
        config.set("diagnostics", "qte_feedback_enabled", "false")  # 离线控制流程测试禁止真实截图线程。
        frame = np.zeros((24, 300, 3), dtype=np.uint8)
        mask = np.full((24, 300), 255, dtype=np.uint8)
        for strategy_class in (qte_strategy.FrostStraitQTEStrategy, qte_strategy.AbyssMawQTEStrategy):
            with self.subTest(strategy=strategy_class.__name__), patch("builtins.print"), \
                    patch.object(qte_strategy.pydirectinput, "press") as press, \
                    self.assertLogs("diagnostic_logging", level="DEBUG") as logs:
    ...
    def test_repeated_rectangles_are_counted_and_changes_survive_summary(self):
        ...
        with patch("diagnostic_logging.time.monotonic", clock), \
                self.assertLogs("diagnostic_logging", level="DEBUG") as logs:
            trace = QTETrace()
            for _ in range(100):
    ...
    def test_detailed_samples_are_debug_only_and_off_by_default(self):
        with self.assertLogs("diagnostic_logging", level="DEBUG") as logs:
            default = QTETrace()
    ...
    def test_cancellation_and_exception_flush_last_partial_window(self):
        ...
        for error, reason in ((run_control.RunStopped(), "cancelled"), (ValueError("bad mask"), "exception:ValueError")):
            strategy = Strategy()
            with self.subTest(reason=reason), self.assertLogs("diagnostic_logging", level="DEBUG") as logs:
    ...
    def test_timeout_is_warning_instead_of_normal_completion(self):
        with self.assertLogs("diagnostic_logging", level="WARNING") as logs:
            trace = QTETrace()
            trace.close()
