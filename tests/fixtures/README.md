...
`qte_control/freeze_20260914/` 增加两份攻略的冰晶计数观察回归：巴哈 3 独立正例、11 张其他机制负例，以及明确标记同源的 GameKee 1/2/3 图。来源、模板处理和哈希见该目录清单；同源图不能替代独立验证，没有自动破冰输入或三阶段连续成功序列。

`qte_control/guide_20260914/wall.png` 为巴哈攻略静态原图的小幅 QTE 裁剪，来源 URL、原图与裁剪哈希、坐标见同目录 `manifest.json`。用于墙体正例及同帧亮指针负例，不是用户游戏捕获或墙体碰撞连续验收。
