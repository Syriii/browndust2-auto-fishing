...
from qte_feedback import EvidenceWriter, FeedbackSession
import qte_strategy
import run_control
import utils
...
class DecisionEvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config.read_string(utils.DEFAULT_CONFIG_CONTENT)
        self.window = utils.Rect(0, 0, 875, 492)
    ...
    def test_control_frame_survives_buffer_reuse_and_is_distinct_from_feedback_frame(self):
        ...
        decision = dict(reason="yellow_overlap", captured_at_monotonic=9.9,
                        frame_region=[300, 400, 380, 430], cursor_x=10)
        with tempfile.TemporaryDirectory() as directory:
            session.writer = EvidenceWriter(directory, self.config, session.region, self.window,
                                            round_id=session.round_id)
            with patch("qte_feedback.time.monotonic", return_value=10):
                session.begin_press(decision, control)
            control[:] = 0
            decision["reason"] = "changed_after_call"
            session.samples.append((10.1, feedback))
            session.publish(session.tracker.observe("miss", 10.1, .95))
            session.close()
            with ZipFile(next(Path(directory).glob("*.zip"))) as archive:
                ...
                decode = lambda name, flag=1: cv2.imdecode(np.frombuffer(archive.read(name), np.uint8), flag)
                ...
                for name in ("white", "yellow", "blue"):
                    color = utils.read_hsv_range(self.config, "roi", name)
                    np.testing.assert_array_equal(decode(f"decision_{name}.png", 0),
    ...
    def test_rapid_attempts_keep_their_own_decisions_and_unassigned_fail_gets_none(self):
        ...
        for stamp, reason in ((10, "yellow_overlap"), (10.2, "blue_fallback")):
            with patch("qte_feedback.time.monotonic", return_value=stamp):
                session.begin_press(dict(reason=reason), np.full((4, 5, 3), int(stamp * 10), np.uint8))
        session.publish(session.tracker.observe("hit", 10.3, .95))
        session.samples.append((11, np.zeros((4, 5, 3), np.uint8)))
        session.publish(session.tracker.observe("fail", 11, .95))
        ...
        self.assertEqual([r["decision"]["reason"] for r in records[:2]], ["yellow_overlap", "blue_fallback"])
    ...
    def test_confirmed_hit_releases_frame_but_retains_decision_metadata(self):
        ...
        session.publish(session.tracker.observe("critical", 10.1, .95))
        self.assertFalse(session.press_decisions)
        self.assertFalse(session.pending_evidence)
    ...
    def test_all_existing_press_branches_and_no_press_keep_input_behavior(self):
        cases = [(qte_strategy.FrostStraitQTEStrategy, reason) for reason in
                 ("yellow_overlap", "no_cursor_fallback", "ice_break_attempt", "no_press")]
        cases += [(qte_strategy.AbyssMawQTEStrategy, reason) for reason in
                  ("yellow_overlap", "blue_fallback", "no_press")]
        for cls, reason in cases:
            for enabled in (False, True):
                with self.subTest(strategy=cls.__name__, reason=reason, enabled=enabled):
                    ...
                    observer = Mock()
                    strategy._start_feedback = lambda: setattr(strategy, "_feedback_session", observer if enabled else None)
                    strategy._sleep_loop = Mock(side_effect=run_control.RunStopped("end sample"))
                    raw = np.full((strategy.roi_pos.height, strategy.roi_pos.width, 3), 80, np.uint8)
                    ...
                    if cls is qte_strategy.AbyssMawQTEStrategy:
                        strategy._blue_mask = Mock(return_value=cursor if reason == "blue_fallback" else np.zeros_like(cursor))
                        strategy._blocker_rect = Mock(return_value=None)
                    with patch.object(qte_strategy.pydirectinput, "press") as press, patch("run_control.sleep"):
