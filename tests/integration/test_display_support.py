...
from tests.support import DEFAULT_CONFIG
...
class DisplayTests(unittest.TestCase):
    ...
    def test_camera_uses_matching_gpu_and_output_and_releases(self):
        for region, device, output, local in (
            (geometry.Rect(-2400, -100, -1300, 600), 0, 1, (160, 100, 1260, 800)),
            (geometry.Rect(2100, 50, 3200, 700), 1, 0, (180, 50, 1280, 700)),
        ):
    ...
    def test_factory_indices_skip_empty_adapters_but_keep_offline_output_slots(self):
        ...
        with patch.dict(
            sys.modules,
            {
                "dxcam.util.io": SimpleNamespace(
                    enum_dxgi_adapters=lambda: adapters, enum_dxgi_outputs=lambda adapter: adapter
                ),
        ...
        self.assertEqual(
            [(o.device_idx, o.output_idx, o.name) for o in result], [(0, 1, "a"), (1, 0, "b")]
        )
    ...
    def test_default_and_source_configs_enable_automatic_cleanup(self):
        ...
        config.read(DEFAULT_CONFIG, encoding="utf-8-sig")
        self.assertTrue(config.getboolean("backpack", "auto_clear_enabled"))
...
class MouseAndWindowTests(unittest.TestCase):
    ...
    def test_click_coordinates_cannot_reenter_primary_screen_conversion(self):
        with (
            patch.object(controlled_input, "moveTo") as move,
    ...
    def test_window_movement_prevents_next_input_but_cleanup_still_runs(self):
        region = geometry.Rect(0, 0, 1000, 700)
        with (
            patch.object(window.win32gui, "FindWindow", return_value=1),
            patch.object(window.win32gui, "IsWindow", return_value=True),
            patch.object(window.win32gui, "IsIconic", return_value=False),
            patch.object(window, "_client_region", return_value=region) as current,
        ):
            guard = window.WindowGuard("test", region)
            ...
            with run_control.use_control(control), run_control.use_input_guard(guard):
                current.return_value = geometry.Rect(1920, 0, 2920, 700)
    ...
    def test_minimized_window_rejects_session(self):
        with (
            patch.object(window.win32gui, "FindWindow", return_value=1),
            patch.object(window.win32gui, "IsWindow", return_value=True),
            patch.object(window.win32gui, "IsIconic", return_value=True),
        ):
            with self.assertRaisesRegex(RuntimeError, "最小化"):
                window.WindowGuard("test", geometry.Rect(0, 0, 1000, 700))()
