...
from bd2_fishing.infrastructure.settings import update_config_options
from bd2_fishing.infrastructure.windows import session as session_support
...
from bd2_fishing.ui.window import UILogHandler
...
class AppSessionTests(unittest.TestCase):
    def test_gui_manual_location_does_not_need_ocr_or_console(self):
        ...
        bot = fishing_task.FishingBot(
            config,
            geometry.Rect(0, 0, 1152, 648),
            Mock(),
            location=FishingLocation.ABYSS_MAW,
            interactive=False,
        )
        with (
            patch.object(fishing_task.island_reading, "detect_location_from_ocr") as detect,
    ...
    def test_gui_ocr_failure_requests_page_selection_without_console_wait(self):
        ...
        config.read_string(settings.DEFAULT_CONFIG_CONTENT)
        bot = fishing_task.FishingBot(
            config, geometry.Rect(0, 0, 1152, 648), Mock(), interactive=False
        )
        with (
            patch.object(
                fishing_task.island_reading, "detect_location_from_ocr", return_value=None
            ),
            patch.object(run_control, "console_input") as prompt,
        ):
    ...
    def test_foreground_guard_treats_focus_loss_as_user_stop(self):
        with (
            ...
            patch.object(window.win32gui, "GetForegroundWindow", return_value=2),
        ):
            guard = window.WindowGuard(
                "test", geometry.Rect(0, 0, 100, 100), require_foreground=True
            )
    ...
    def test_ui_saves_only_owned_options_and_preserves_comments(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            config = update_config_options(
                path,
                {
                    "backpack": {"auto_clear_enabled": "false"},
                    "app": {"prevent_sleep": "true", "location": "深渊巨口"},
                },
            )
    ...
    def test_denied_focus_does_not_start_gameplay(self):
        with (
            ...
            patch.object(
                session_support.win32gui, "SetForegroundWindow", side_effect=OSError("denied")
            ),
    ...
    def test_locked_desktop_rejects_focus_attempt(self):
        with (
            patch.object(session_support, "input_desktop_available", return_value=False),
            patch.object(session_support.win32gui, "SetForegroundWindow") as focus,
        ):
    ...
    def test_power_request_restores_previous_state_after_cancellation(self):
        ...
        with patch.object(
            session_support.ctypes.windll.kernel32, "SetThreadExecutionState", setter
        ):
    ...
    def test_disabled_power_option_leaves_system_alone(self):
        with patch.object(
            session_support.ctypes.windll.kernel32, "SetThreadExecutionState"
        ) as setter:
