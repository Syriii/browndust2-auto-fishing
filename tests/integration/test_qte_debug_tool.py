...
from scripts.live import record_qte_session as tool


class DebugConfigTests(unittest.TestCase):
    def test_default_uses_local_config_and_ignores_parent_deployment(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "auto_fishing-dev"
            root.mkdir()
            path = root / ".local" / "config.ini"
            path.parent.mkdir()
            (root.parent / "config.ini").write_text("unrelated deployment", encoding="utf-8")
