...
from bd2_fishing.infrastructure.windows.gdi import FeedbackCapture, _BitmapInfoHeader
...
class FeedbackCaptureTests(unittest.TestCase):
    def backend(self):
        ...
        def create(screen, header, usage, target, section, offset):
            ...
            ctypes.cast(target, ctypes.POINTER(ctypes.c_void_p)).contents.value = ctypes.addressof(
                pixels
            )
            return 33
    ...
    def test_bgr_is_owned_copy_and_handles_release_once(self):
        ...
        with patch("bd2_fishing.infrastructure.windows.gdi.ctypes.WinDLL", side_effect=[gdi, user]):
