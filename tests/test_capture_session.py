...
class CaptureSessionTests(unittest.TestCase):
    def output(self, name, bounds):
        return SimpleNamespace(attached_to_desktop=True, devicename=name, update_desc=Mock(),
    ...
    def modules(self, adapters, constructor, legacy):
        return {
            ...
            "dxcam.util.io": SimpleNamespace(enum_dxgi_adapters=adapters, enum_dxgi_outputs=lambda adapter: adapter),
        }
    ...
    def test_new_right_output_ignores_short_cached_factory_and_reenumerates_on_restart(self):
        ...
        region = utils.Rect(1936, 577, 2811, 1069)
        with patch.dict(sys.modules, self.modules(adapters, constructor, legacy)), patch.object(run_control, "sleep"):
    ...
    def test_cancel_after_creation_releases_unreturned_camera(self):
        ...
        camera = Mock()
        with patch.dict(sys.modules, self.modules(lambda: [[output]], Mock(return_value=camera), Mock())), \
    ...
    def test_layout_changes_during_creation_do_not_use_stale_coordinates(self):
        output = self.output("primary", (0, 0, 1920, 1080))
        output.update_desc.side_effect = lambda: setattr(output.desc.DesktopCoordinates, "left", 1920)
        constructor = Mock()
        with patch.dict(sys.modules, self.modules(lambda: [[output]], constructor, Mock())), \
    ...
    def test_missing_output_has_actionable_error_without_creating_camera(self):
        constructor = Mock()
        with patch.dict(sys.modules, self.modules(lambda: [], constructor, Mock())), \
