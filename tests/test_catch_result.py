...
import utils
import qte_strategy
from catch_result import CatchObserver, classify_settlement, read_settlement_texts, run_observed_qte
from qte_feedback import FeedbackSession
from ocr.ocr_engine import OCRText, OCRBox

FIXTURES = Path(__file__).parent / "fixtures/catch_result"
...
class CatchResultTests(unittest.TestCase):
    ...
    def test_reward_requires_panel_quantity_and_size(self):
        reward = [OCRText("鱼×1", .86), OCRText("14.7cm", .99)]
        ...
        self.assertEqual(classify_settlement(False, reward, [1,1], [OCRText("90CM", .99)]).status, "unknown")
        self.assertEqual(classify_settlement(True, reward[:1], [1, 1], [OCRText("90cm", .95)]).status, "unknown")
    ...
    def test_empty_scene_and_single_low_timer_do_not_prove_escape(self):
        ...
        self.assertEqual(classify_settlement(False, [], [1], [OCRText("90CM", .95)]).status, "unknown")

    def test_timeout_is_explicitly_an_inference(self):
        result = classify_settlement(False, [], [2,1,1], [OCRText("9OCM", .84)])
        self.assertEqual((result.status, result.remaining_cm), ("suspected_escape", 90))
        self.assertEqual(classify_settlement(False, [], [2,2,1], [OCRText("90CM", .95)]).status, "suspected_escape")
        self.assertEqual(classify_settlement(False, [], [1,1], [OCRText("0CM", .99)]).status, "unknown")
        self.assertEqual(classify_settlement(False, [], [1,1], [OCRText("90CM", .60)]).status, "unknown")
    ...
    def test_real_settlement_panel_and_normal_scene_are_distinct(self):
        ...
        for name, expected in (("caught", True), ("caught_unusual_name", True), ("normal", False), ("escaped_scene", False), ("last_qte", False)):
            frame = cv2.imdecode(np.frombuffer((FIXTURES/f"{name}.png").read_bytes(),np.uint8),1)
    ...
    def test_timer_ignores_uncertain_text_and_stops_reading_during_settlement(self):
        engine = Mock()
        engine.recognize.side_effect = [OCRText("1", .98), OCRText("1", .4), OCRText("c", .99)]
        observer = CatchObserver(engine, self.config(), utils.Rect(0,0,875,492))
    ...
    def test_settlement_uses_existing_wait_budget_and_preserves_click(self):
        strategy = qte_strategy.FrostStraitQTEStrategy(self.config(),utils.Rect(0,0,875,492))
        strategy.fish_end_wait_time = 4
        strategy.catch_observer = Mock()
        with patch.object(qte_strategy.time,"monotonic",side_effect=[10,11.5]), \
    ...
    def test_uncertain_fish_name_can_verify_quantity_without_inventing_name(self):
        box=OCRBox(((0,0),(45,0),(45,14),(0,14)),.65)
        engine=Mock()
        engine.detect_and_recognize.return_value=[OCRText("螺x1",.65,box)]
        engine.recognize.side_effect=[OCRText("端螺×1",.56),OCRText("x1",.81)]
        result=read_settlement_texts(engine,np.zeros((30,70,3),np.uint8))
        self.assertEqual((result[0].text,result[0].score),("×1",.81))

    def test_refinement_disagreement_does_not_invent_distance(self):
        box=OCRBox(((0,0),(25,0),(25,12),(0,12)),.70)
        engine=Mock()
        engine.detect_and_recognize.return_value=[OCRText("90CM",.70,box)]
        engine.recognize.return_value=OCRText("900CM",.99)
        result=read_settlement_texts(engine,np.zeros((30,70,3),np.uint8))
        self.assertEqual((result[0].text,result[0].score),("90CM",.70))
    ...
    def test_all_exit_paths_save_final_pending_key_and_cached_frame(self):
        for error, status in ((None, "unknown"), (run_control.RunStopped("失焦"), "interrupted"),
                              (ValueError("test"), "unknown")):
            with self.subTest(status=status, error=error), tempfile.TemporaryDirectory() as directory:
                observer = CatchObserver(Mock(recognize=Mock(return_value=None)), self.config(), utils.Rect(0,0,875,492))
                ...
                with patch("catch_result.utils.get_base_path", return_value=directory), \
                     patch("catch_result.FeedbackCapture") as capture:
                    if error is None:
                ...
                with ZipFile(archives[0]) as archive:
                    ...
                    saved = cv2.imdecode(np.frombuffer(archive.read("last_observed_qte.png"), np.uint8), 1)
                self.assertTrue(np.array_equal(saved, frame))
                self.assertEqual(metadata["result"]["status"], status)
                self.assertEqual([(item["attempt"], item["result"]) for item in metadata["attempt_outcomes"]], [(1, "unknown")])
    ...
    def test_ocr_failure_keeps_captured_settlement_and_does_not_block_original_click(self):
        ...
        with tempfile.TemporaryDirectory() as directory, \
             ...
             patch("catch_result.utils.WindowGuard"), patch("catch_result.FeedbackCapture") as capture, \
             patch.object(observer, "_panel_open", return_value=True), \
             patch("catch_result.read_settlement_texts", side_effect=ValueError("OCR test")), \
             patch.object(run_control, "sleep"), patch.object(qte_strategy.pydirectinput, "moveTo"), \
             patch.object(qte_strategy.pydirectinput, "click") as click:
            ...
            with self.assertLogs("qte_strategy", level="ERROR"):
                run_observed_qte(strategy, None)
    ...
    def test_no_frames_or_keys_still_leave_an_explicit_unknown_record(self):
        with tempfile.TemporaryDirectory() as directory, patch("catch_result.utils.get_base_path", return_value=directory):
            observer = CatchObserver(Mock(), self.config(), utils.Rect(0,0,875,492))
