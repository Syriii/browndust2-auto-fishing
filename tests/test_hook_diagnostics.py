...
class HookDiagnosticsTests(unittest.TestCase):
    ...
    def test_peak_survives_source_mutation_and_reset(self):
        ...
        with ZipFile(self.output / "timeout_01.zip") as archive:
            peak = cv2.imdecode(np.frombuffer(archive.read("peak_hook.png"), np.uint8), cv2.IMREAD_COLOR)
            last = cv2.imdecode(np.frombuffer(archive.read("last_hook.png"), np.uint8), cv2.IMREAD_COLOR)
    ...
    def test_slow_encoding_is_async_and_busy_writer_drops_new_work(self):
        ...
        with patch("hook_diagnostics.cv2.imencode", side_effect=slow_encode):
            try:
                self.assertTrue(self.save())
    ...
    def test_write_failure_releases_writer_and_allows_future_save(self):
        self.recorder.observe(self.peak, 180)
        with patch("hook_diagnostics.cv2.imencode", side_effect=OSError("disk unavailable")):
            with self.assertLogs("hook_diagnostics", level="WARNING"):
                self.save()
                self.join()
        self.assertTrue(self.save())
        self.join()
...
class TimeoutIntegrationTests(unittest.TestCase):
    def test_snapshot_is_submitted_before_recovery_input(self):
        ...
        namespace = {
            ...
            "log": Mock(), "print": Mock(),
            "run_control": Mock(),
            "operate": SimpleNamespace(recover_from_timeout=recover),
        }
        module = ast.Module(body=[ast.ImportFrom(module="__future__", names=[ast.alias(name="annotations")], level=0), method], type_ignores=[])
        exec(compile(ast.fix_missing_locations(module), str(source), "exec"), namespace)
        diagnostics = Mock()
        diagnostics.save_timeout.side_effect = lambda *args: events.append("snapshot")
        bot = SimpleNamespace(hook_diagnostics=diagnostics, bite_pixel_threshold=212,
                              region=None, hook_pos=None, selected_location_name="深渊巨口",
