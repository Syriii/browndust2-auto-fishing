...
import numpy as np
import qte_strategy
from qte_feedback import EvidenceWriter, FeedbackMatcher, Outcome, OutcomeTracker, select_evidence_frames
import run_control
import utils

FIXTURES = Path(__file__).parent / "fixtures" / "qte_feedback"
    ...
    def test_real_words_and_backgrounds(self):
        ...
        for sample in json.loads((FIXTURES / "manifest.json").read_text(encoding="utf-8")):
            with self.subTest(file=sample["file"]):
                frame = cv2.imdecode(np.frombuffer((FIXTURES/sample["file"]).read_bytes(), np.uint8), 1)
...
class OutcomeTests(unittest.TestCase):
    def test_each_new_word_maps_to_actual_feedback_not_geometry(self):
        for label, expected in (("critical","critical"),("hit","hit"),("miss","miss"),("fail","miss")):
            with self.subTest(label=label):
                tracker = OutcomeTracker()
                self.assertEqual(tracker.begin(0), [])
                result, = tracker.observe(label, .1, .95)
                self.assertEqual((result.attempt, result.result, result.feedback), (1,expected,label))
                self.assertEqual(tracker.close(.5), [])

    def test_old_animation_cannot_confirm_second_press(self):
        t = OutcomeTracker()
        t.begin(0)
        self.assertEqual(t.observe("critical", .1)[0].result,"critical")
        t.begin(.3)
        self.assertEqual(t.observe("critical", .35), [])
        self.assertEqual(t.observe("critical", .7), [])  # 无图时间间隔不能让旧动画重新生效
        self.assertEqual(t.close(.9)[0].result, "unknown")

    def test_blank_interval_rearms_same_word(self):
        t = OutcomeTracker(); t.begin(0); t.observe("hit",.1)
        t.observe(None,.25); t.observe(None,.4); t.begin(.5)
        self.assertEqual(t.observe("hit",.6)[0].attempt,2)

    def test_single_bad_frame_does_not_duplicate_animation(self):
        t = OutcomeTracker(); t.begin(0); t.observe("hit",.1)
        t.observe(None,.12); t.begin(.13)
        self.assertEqual(t.observe("hit",.14), [])

    def test_bad_frame_followed_by_capture_gap_cannot_rearm_animation(self):
        t = OutcomeTracker(); t.begin(0); t.observe("hit", .1)
        t.observe(None, .12); t.begin(.3)
        self.assertEqual(t.observe("hit", .4), [])
        self.assertEqual(t.close(.5)[0].result, "unknown")

    def test_timeout_without_images_does_not_count_as_blank(self):
        t = OutcomeTracker(); t.begin(0); t.observe("critical", .1); t.begin(.3)
    ...
    def test_rapid_unconfirmed_presses_remain_ambiguous(self):
        t = OutcomeTracker(); t.begin(0)
        self.assertEqual(t.begin(.3)[0].result,"unknown")
        result, = t.observe("critical",.4)
    ...
    def test_confirmed_game_feedback_is_retained_even_when_press_is_ambiguous(self):
        t = OutcomeTracker(); t.begin(0); t.begin(.3)
        self.assertEqual(t.observe("hit", .4)[0].result, "unknown")
    ...
    def test_expired_old_candidate_does_not_hide_new_hit(self):
        t = OutcomeTracker(); t.begin(0); t.begin(.7)
        result, = t.observe("hit", .78)
...
class IntegrationTests(unittest.TestCase):
    ...
    def test_existing_input_call_is_unchanged_and_can_disable_observer(self):
        ...
        strategy=qte_strategy.FrostStraitQTEStrategy(config,utils.Rect(0,0,875,492))
        self.assertFalse(strategy.feedback_enabled)
        strategy._start_feedback()
        self.assertIsNone(strategy._feedback_session)
        observer=Mock(); strategy._feedback_session=observer
    ...
    def test_source_config_and_defaults_enable_observer_consistently(self):
        ...
        source.read(Path(__file__).resolve().parents[1]/"config.ini",encoding="utf-8-sig")
        ...
        self.assertTrue(qte_strategy.FrostStraitQTEStrategy(source,utils.Rect(0,0,875,492)).feedback_enabled)
    ...
    def test_observer_is_closed_once_when_qte_ends(self):
        ...
        observer=Mock(); strategy._feedback_session=observer
        strategy._stop_feedback(); strategy._stop_feedback()
        observer.close.assert_called_once_with()
        self.assertIsNone(strategy._feedback_session)

    def test_observer_error_does_not_suppress_input(self):
        strategy=qte_strategy.FrostStraitQTEStrategy(self.config(),utils.Rect(0,0,875,492))
        strategy._feedback_session=Mock(begin_press=Mock(side_effect=ValueError("diagnostic error")))
        with patch.object(qte_strategy.pydirectinput,"press") as press, self.assertLogs("qte_strategy",level="ERROR"):
            strategy._press_qte()
        press.assert_called_once_with("space")

    def test_input_stop_still_propagates(self):
        strategy=qte_strategy.FrostStraitQTEStrategy(self.config(),utils.Rect(0,0,875,492))
        with patch.object(qte_strategy.pydirectinput,"press",side_effect=run_control.RunStopped("stop")):
            with self.assertRaises(run_control.RunStopped):
                strategy._press_qte()

    def test_evidence_contains_same_frame_masks_and_coordinates(self):
        with tempfile.TemporaryDirectory() as directory:
            writer=EvidenceWriter(directory,self.config(),utils.Rect(262,315,612,458),utils.Rect(0,0,875,492))
            frame=cv2.imdecode(np.frombuffer((FIXTURES/"miss.png").read_bytes(),np.uint8),1)
            writer.submit(Outcome(1,1,1.1,"miss","miss","实图反馈",.95),[(.95,frame),(1.1,frame)])
            writer.close()
    ...
    def test_evidence_keeps_feedback_and_press_neighbors_when_downsampling(self):
        frames = [(i*.02, None) for i in range(40)]
        outcome = Outcome(1, .215, .54, "miss", "fail", "test")
        selected = select_evidence_frames(frames, outcome)
        stamps = [stamp for stamp, _ in selected]
        self.assertLessEqual(len(selected), 8)
        for stamp in (0, .20, .22, .54, .78):
