...
class CastFeedbackTests(unittest.TestCase):
    def check_feedback(self, texts):
        ...
        engine = Mock(
            detect_and_recognize=Mock(
                return_value=[SimpleNamespace(text=text, score=0.97732, box=None) for text in texts]
            )
        )
        ...
        return cast_feedback.check_backpack_if_full(capture, context)

    def test_recorded_cast_failure_signals_recovery_and_exposes_original_text(self):
        text = "当前位置无法抛竿。请移至船边重新尝试。"
        with self.assertLogs("bd2_fishing.game.fishing.cast_feedback", level="WARNING") as logs:
            with self.assertRaisesRegex(cast_feedback.CastPositionBlocked, "当前位置无法抛竿"):
                self.check_feedback([text])
        self.assertIn(text, logs.output[-1])

    def test_split_cast_failure_still_signals_recovery(self):
        with (
            self.assertLogs("bd2_fishing.game.fishing.cast_feedback", level="WARNING"),
            self.assertRaises(cast_feedback.CastPositionBlocked),
        ):
