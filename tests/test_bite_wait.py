...
import numpy as np
import run_control
from ocr.ocr_service import CastPositionBlocked
...
class BiteWaitTests(unittest.TestCase):
    def setUp(self):
        ...
        self.bot = SimpleNamespace(
            hook_diagnostics=self.diagnostics, bite_pixel_threshold=212,
            region=None, hook_pos=None, selected_location_name="深渊巨口",
            config=None, ocr_context=None,
        ...
        self.namespace = {
            ...
            "BITE_TIMEOUT_SECONDS": 15, "log": Mock(), "print": Mock(),
            "run_control": run_control, "cv2": cv2,
            "utils": SimpleNamespace(create_color_mask=lambda *a, **k: self.mask()),
            "operate": SimpleNamespace(recover_from_timeout=self.recover,
                                       clear_backpack=lambda *a: self.advance(20),
                                       cast_rod=self.cast),
            "ocr_service": SimpleNamespace(check_backpack_if_full=self.backpack, CastPositionBlocked=CastPositionBlocked),
    ...
    def test_disabled_cleanup_stops_on_full_backpack_without_input(self):
        ...
        clear = Mock()
        self.namespace["operate"].clear_backpack = clear
