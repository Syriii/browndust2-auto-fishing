...
import run_control
from diagnostic_logging import trace_qte
...
class BaseQTEStrategy:
    ...
    def __init__(self, config: configparser.ConfigParser, region: Rect) -> None:
        ...
        self.feedback_enabled = config.getboolean("diagnostics", "qte_feedback_enabled", fallback=True)
    ...
    def _start_feedback(self):
        ...
        try:
            ...
            self._feedback_session = FeedbackSession(self._feedback_config, self.region,
                                                    getattr(self, "catch_observer", None))
        ...
        if self._feedback_session is not None:
            try:
                if reason is None:
                    ...
                    decision = dict(strategy=type(self).__name__, reason=reason,
                        stage="before_input_call", captured_at_monotonic=self._decision_captured_at,
                        ...
                        press_tolerance_pixels=self.press_tolerance_pixels, **details)
    ...
    def _finish_fishing(self) -> None:
        ...
        if catch_observer is not None:
            try:
                ...
                from catch_result import CatchResult
                catch_observer.result = CatchResult("unknown", f"结算观察失败：{type(exc).__name__}")
...
class FrostStraitQTEStrategy(BaseQTEStrategy):
    ...
    def play_qte(self, sct: utils.DxCameraCapture) -> None:
        ...
        while time.monotonic() - start_time < self.longest_keep_time:
            ...
            if check_x is not None and self._mask_column_has_color(mask_yellow, check_x):
                self._press_qte("yellow_overlap", cursor_x=cursor_x, check_x=check_x, target="yellow")
                pressed = True
    ...
    def solve_ice_trouble(self, roi_hsv: np.ndarray) -> bool:
        ...
        if cv2.countNonZero(mask) > self.ice_trouble_pixel_threshold:
            self._press_qte("ice_break_attempt", target="ice", red_pixels=cv2.countNonZero(mask),
        ...
        while time.monotonic() - start_time < self.longest_keep_time:
            ...
            if yellow_pixels > self.abyss_yellow_pixel_threshold:
                if self._mask_column_has_color(yellow_mask, check_x):
                    self._press_qte("yellow_overlap", cursor_x=cursor_x, check_x=check_x,
                                    target="yellow", active_range=(left_x, right_x),
                                    blocker_rect=blocker_rect, yellow_pixels=yellow_pixels,
                                    yellow_threshold=self.abyss_yellow_pixel_threshold)
                    pressed = True
            elif self._mask_column_has_color(blue_mask, check_x):
                self._press_qte("blue_fallback", cursor_x=cursor_x, check_x=check_x,
                                target="blue", active_range=(left_x, right_x),
                                blocker_rect=blocker_rect, yellow_pixels=yellow_pixels,
                                yellow_threshold=self.abyss_yellow_pixel_threshold)
                pressed = True
            self._qte_trace.observe("tracking", blocker=blocker_rect, cursor=cursor_x,
                                    yellow=yellow_pixels, active=(left_x, right_x), pressed=pressed)
