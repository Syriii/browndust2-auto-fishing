...
def update_config_options(path, updates):
    ...
    path = Path(path)
    text = path.read_text(encoding="utf-8-sig") if path.exists() else utils.DEFAULT_CONFIG_CONTENT
...
class FishingApp:
    def __init__(self, root, target, *, preview=False):
        self.root, self.target, self.preview = root, target, preview
        self.config_path = Path(utils.get_base_path()) / "config.ini"
        ...
        self.controller = run_control.TaskController(self._run, (lambda: None) if preview else controlled_input.release_inputs)
        ...
        self.auto_clear = tk.BooleanVar(value=config.getboolean("backpack", "auto_clear_enabled", fallback=True))
        self.awake = tk.BooleanVar(value=config.getboolean("app", "prevent_sleep", fallback=False))
        self.detail_log = tk.BooleanVar(value=config.getboolean("diagnostics", "qte_detail_log", fallback=False))
        self.feedback = tk.BooleanVar(value=config.getboolean("diagnostics", "qte_feedback_enabled", fallback=True))
    ...
    def _build(self, locations):
        ...
        style.configure("TLabel", background="white", foreground="#253249", font=("Microsoft YaHei UI", 10))
        ...
        style.configure("Start.TButton", background="#117568", foreground="white", font=("Microsoft YaHei UI", 11, "bold"))
        style.map("Start.TButton", background=[("disabled", "#ccd6da"), ("active", "#0c6258")])
        outer = ttk.Frame(root, padding=22)
        outer.pack(fill="both", expand=True)
        header = ttk.Frame(outer)
        header.pack(fill="x", pady=(0, 14))
        tk.Label(header, text="BD2 自动钓鱼", bg="#eef2f6", fg="#162b42",
                 font=("Microsoft YaHei UI", 21, "bold")).pack(side="left")

        card = ttk.Frame(outer, style="Card.TFrame", padding=18)
        card.pack(fill="x", pady=(0, 12))
        self.status_label = ttk.Label(card, textvariable=self.status, font=("Microsoft YaHei UI", 15, "bold"))
        self.status_label.pack(anchor="w")
        ttk.Label(card, textvariable=self.detail, style="Hint.TLabel", wraplength=760).pack(anchor="w", pady=(5, 12))
        buttons = ttk.Frame(card, style="Card.TFrame")
        buttons.pack(fill="x")
        self.start_button = ttk.Button(buttons, text="开始钓鱼", style="Start.TButton", command=self.start)
        self.start_button.pack(side="left", padx=(0, 10))
        self.stop_button = ttk.Button(buttons, text="停止任务", command=self.stop, state="disabled")
        self.stop_button.pack(side="left")
        ttk.Label(buttons, text="游戏需保持前台可见；锁屏后无法继续", style="Hint.TLabel").pack(side="right")
        ...
        self.location_box = ttk.Combobox(settings, values=locations, textvariable=self.location, state="readonly", width=17)
        self.location_box.grid(row=0, column=1, sticky="w", padx=(0, 22))
        self.clear_check = ttk.Checkbutton(settings, text="满包时自动清理", variable=self.auto_clear)
        self.clear_check.grid(row=0, column=2, sticky="w")
        self.awake_check = ttk.Checkbutton(settings, text="运行时防止自动息屏 / 睡眠", variable=self.awake)
        self.awake_check.grid(row=1, column=0, columnspan=2, sticky="w", pady=(12, 0))
        self.trace_check = ttk.Checkbutton(settings, text="逐帧诊断写入文件", variable=self.detail_log)
        self.trace_check.grid(row=1, column=2, sticky="w", pady=(12, 0))
        self.feedback_check = ttk.Checkbutton(settings, text="记录 QTE 结果、失败截图和鱼获结算", variable=self.feedback)
        ...
        ttk.Label(toolbar, text="运行日志", font=("Microsoft YaHei UI", 11, "bold")).pack(side="left")
        selector = ttk.Combobox(toolbar, textvariable=self.level, values=["运行信息", "警告 / 错误", "详细诊断"], state="readonly", width=13)
        ...
        self.text = tk.Text(text_frame, bg="#f7f9fb", fg="#344155", relief="flat", wrap="word",
                            font=("Microsoft YaHei UI", 9), padx=10, pady=10, state="disabled", height=10)
        ...
        ttk.Label(panel, textvariable=self.issue, wraplength=750, style="Hint.TLabel").pack(anchor="w", pady=(8, 0))
        ttk.Label(outer, text="界面保留最近 1500 条；完整日志保存在程序目录，按大小轮转。",
                  background="#eef2f6", style="Hint.TLabel").pack(anchor="w", pady=(10, 0))
    ...
    def start(self):
        ...
        try:
            if not self.preview:
                self.snapshot = update_config_options(self.config_path, {
                    "backpack": {"auto_clear_enabled": str(self.auto_clear.get()).lower()},
                    "diagnostics": {"qte_detail_log": str(self.detail_log.get()).lower(),
                                    "qte_feedback_enabled": str(self.feedback.get()).lower()},
                    "app": {"location": self.location.get(), "prevent_sleep": str(self.awake.get()).lower()},
                })
            self.selected_location = None if self.location.get() == "自动识别" else FishingLocation(self.location.get())
    ...
    def _poll(self):
        ...
        if self.closing:
            ...
            self.detail.set(self.controller.last_error or self.controller.last_stop_reason or "点击开始，自动聚焦游戏后运行。")
    ...
    def _append(self, records):
        minimum = {"运行信息": logging.INFO, "警告 / 错误": logging.WARNING, "详细诊断": logging.DEBUG}[self.level.get()]
        position = self.text.yview()[0]
        self.text.configure(state="normal")
        for level, message in records:
            if level >= minimum:
                tag = "ERROR" if level >= logging.ERROR else "WARNING" if level >= logging.WARNING else "DEBUG" if level < logging.INFO else "INFO"
