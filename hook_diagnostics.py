        ...
        try:
            ...
            if peak is not None:
                ...
                metadata["peak_filter_counts"] = {
                    "hue_only": cv2.countNonZero(cv2.inRange(
                        hsv, (lower[0], 0, 0), (upper[0], 255, 255))),
                    "hue_and_saturation": cv2.countNonZero(cv2.inRange(
                        hsv, (lower[0], lower[1], 0), (upper[0], upper[1], 255))),
                    "hue_and_value": cv2.countNonZero(cv2.inRange(
                        hsv, (lower[0], 0, lower[2]), (upper[0], 255, upper[2]))),
            ...
            job_log.info("上钩超时诊断已保存: %s (峰值=%d，有效帧=%d，无新图=%d)",
                     path, metadata["peak_pixels"], metadata["valid_frames"], metadata["none_frames"])
        except Exception:
