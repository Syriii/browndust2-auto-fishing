...
**版本说明：** 当前源码为 **0.4.5**，图鉴与鱼获采用左侧密集小图／列表、右侧固定详情；鱼获统一使用图鉴图片，补齐已核实的名称和图标关联。钓鱼页恢复常驻运行日志。图鉴和鱼获连续选鱼时复用卡片与详情，回页保留内容，图片缓存淘汰后详情仍可见；本地 EXE 和官方 Release 已完成核验，详见[当前状态](docs/development/status.md)。
...
![0.4.4 实际 Tk 界面：运行日志默认显示](docs/images/ui-0.4.4-run.png)

![0.4.4 实际 Tk 界面：鱼获按日期排列，右侧固定详情](docs/images/ui-0.4.4-catches.png)
...
| 鱼种图鉴 | 六岛 84 种离线鱼图，支持简繁搜索和钓场／昼夜／稀有度筛选；密集小图与列表以图标切换，右侧固定显示详情 |
| 钓鱼目标 | 按任意尺寸、MAX、MIN 或两者选鱼；跨岛并等待所需昼夜，完成的要求移出待办，全部完成后停止 |
| 个人鱼获 | 全部、首次、彩色、最大、最小分别筛选；按本地日期分组并可筛选某天，小图显示尺寸和时间，保留完整历史；旧记录不抵扣新任务 |
...
钓鱼页运行日志框下方的 **更新与存储** 支持检查正式新版或导入本地 ZIP。在线更新失败时，可用浏览器从 Release 下载 ZIP，再在程序中选择该文件，无需解压。更新会保留配置和运行数据。
