...
**版本说明：** 当前源码为 **0.4.5**，图鉴与鱼获采用左侧密集小图／列表、右侧固定详情；鱼获统一使用图鉴图片，补齐已核实的名称和图标关联。钓鱼页恢复常驻运行日志。图鉴和鱼获连续选鱼时复用卡片与详情，回页保留内容，图片缓存淘汰后详情仍可见；EXE 构建与发布状态见[当前状态](docs/development/status.md)。
