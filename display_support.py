...
class DisplayOutput:
    ...
    def local_region(self, region):
        if not self.contains(region):
            raise RuntimeError("截图区域超出所选显示器，请将游戏完整放在一块屏幕内后点击开始钓鱼 重试")
        left, top, right, bottom = region
...
def select_output(outputs, region):
    for output in outputs:
        if output.contains(region):
            return output
    raise RuntimeError("游戏客户区跨屏、超出桌面或没有可用显示器；请完整移到一块屏幕后点击开始钓鱼 重试")
...
def _enumerate_bindings():
    ...
    for adapter in enum_dxgi_adapters():
        ...
        for output_idx, pointer in enumerate(outputs):
            ...
            info = DisplayOutput(device_idx, output_idx, output.devicename,
...
def create_camera_for_region(region, output_color="BGR"):
    ...
    try:
        ...
        if not binding.output.attached_to_desktop or (rect.left, rect.top, rect.right, rect.bottom) != selected.bounds:
            raise RuntimeError("显示器布局在截图初始化期间变化；请等待显示器稳定后重新开始")
        # 有意绕过 dxcam.create 的进程级索引缓存，但沿用其 DXCamera 和后端实现。
        camera = DXCamera(output=binding.output, device=Device(binding.adapter),
                          region=None, output_color=output_color)
...
def normalize_virtual_point(x, y, bounds):
    ...
    return (min(65535, ((2 * (x - left) + 1) * 65536) // (2 * (right - left))),
