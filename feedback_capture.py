...
class _BitmapInfoHeader(ctypes.Structure):
    _fields_ = [("size", w.DWORD), ("width", w.LONG), ("height", w.LONG),
                ...
                ("size_image", w.DWORD), ("x_ppm", w.LONG), ("y_ppm", w.LONG),
...
class FeedbackCapture:
    ...
    def __init__(self, region):
        ...
        signatures = [
            ...
            (self.gdi.CreateDIBSection, [w.HDC, ctypes.c_void_p, w.UINT,
                                       ctypes.POINTER(ctypes.c_void_p), w.HANDLE, w.DWORD], w.HANDLE),
            (self.gdi.BitBlt, [w.HDC, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int,
    ...
    def __enter__(self):
        try:
            ...
            header = _BitmapInfoHeader(size=ctypes.sizeof(_BitmapInfoHeader),
                width=self.region.width, height=-self.region.height, planes=1, bit_count=32)
            ...
            if not self.previous or self.previous == ctypes.c_void_p(-1).value:
                self.previous = None
                raise ctypes.WinError(ctypes.get_last_error())
            buffer = (ctypes.c_uint8 * (self.region.width*self.region.height*4)).from_address(pixels.value)
            self.pixels = np.ctypeslib.as_array(buffer).reshape(self.region.height, self.region.width, 4)
            return self
    ...
    def grab(self):
        ...
        if not self.gdi.BitBlt(self.memory, 0, 0, self.region.width, self.region.height,
                              self.screen, self.region.left, self.region.top, 0x40CC0020):
            raise ctypes.WinError(ctypes.get_last_error())
        self.gdi.GdiFlush()
