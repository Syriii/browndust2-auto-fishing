...
本页保留首轮架构方案及后续追加记录，包含当时的旧路径和未实施目标；当前架构见[架构说明](../design/architecture.md)。
...
仍属目标：完整页面导航、稳定岛屿身份与覆盖清单、组合机制决策、所有设备边界注入、完整任务快照接口、端到端时延基线。默认配置单源化、依赖锁定和运行数据迁移已在[统一布局](../design/repository-layout.md)中实施。当前保留兼容的地点枚举、INI 和原输入时序。部分 game 执行模块仍直接使用基础设施；下文严格依赖图是继续收敛的目标，不能把首轮职责拆分当成全部实现。
...
2026-09-10 已落地的钓鱼内部结构：

```text
game/fishing/
├── qte.py                  共用控制循环、地点目标读取、同步执行及释放
├── mechanics/
│   ├── policy.py           MechanismPolicy + TargetPolicy，统一状态与动作许可
│   ├── regions.py          同帧绿/红/紫/泡泡几何及遮挡排除
│   ├── blue_target.py      原始蓝区、安全内部边界与可信小孔修补
│   ├── green_control.py    有界按住/松开状态
│   ├── bubbles.py          泡泡单次输入许可
│   ├── lifecycle.py        观察侧多外观实例状态，不授权动作或判断解除
│   └── blockers.py         既有深渊单挡板识别与有效范围
├── pointer.py              真光标候选与运动观察
├── trigger_rules.py        普通目标同次入区去重
├── scene_signals.py         独立观察帧的多种外观线索
├── scene_evidence.py        场景事件与前后截图记录
├── feedback*.py            按键反馈归属与观察
└── settlement*.py          整条鱼结算判定与观察
```

`qte.py` 识别控制帧区域后调用纯策略，策略返回唯一专用动作意图；normal 表示可以继续
评估黄蓝普通目标。绿色独占优先，泡泡尝试次之；红紫局部区域仍约束普通输入。
两种地点共用 TargetPolicy 的黄色优先、蓝区连续两帧确认和同次入区去重。
截图缺失、遮挡与不确定边界清除蓝区确认，但不能当作已离开目标而重新授权。

每轮创建状态，调用者传入当前时间；无动作队列、新增线程、等待、同步编码或文件写入。
执行层保留原生按键是否仍待释放的独立标记；纯状态已经释放不代表原生 keyUp 成功。
观察证据和控制帧保持独立，旧观察事件不授权当前输入。

本次没有实现新的机制身份分类或解除确认；绿色实际起按端仍未可靠识别，泡泡许可不代表
击破，挡板仍是原单挡板算法。观察侧已有多外观实例的出现、连续未见及中断记录；
控制侧的技能身份、抵抗提示与解除因果仍未补齐，见[机制链审计](../development/mechanism-chain-audit.md)。

机制属于对应玩法。目前的绿色维持、冰冻、分身指针、遮挡、速度变化等都归 `game.fishing`。以后其他岛屿玩法有自己的机制时归各自功能，先不制造跨玩法的通用机制引擎。

后续继续收敛的职责（不是已经全部建立的目录）：
...
QTE 低时延是架构验收条件，详见[时延设计与验收](../design/qte-performance.md)。控制路径在同一工作线程直接执行，不随模块拆分增加排队。用户选择先优化架构，本轮保留策略与输入时序，以现有回归和策略 AST 对比核对；尚未建立端到端时延基线，不能据此给出毫秒指标或性能合格结论。涉及采样、锁或按键时序的后续改造须先建立测量基线。
...
pyproject 声明直接依赖和构建工具，requirements 保存已验证 Windows Python 3.12 环境的完整锁定清单。模板、默认配置、OCR 模型和 Tcl/Tk 均作为包装验收内容；构建脚本明确拒绝缺少 Tcl/Tk 的产物。工具、测试、CI 和文档的路径随迁移同步更新，详细归属见[统一布局](../design/repository-layout.md)。
...
更新界面通过 app/updates 编排，基础设施中的 updates 子包负责协议、下载、文件事务与独立助手。app/startup 校验配置；infrastructure/maintenance 仅在发布版待机时执行。均不引入 game/runtime 的反向依赖，不参与 QTE 时序。详见[更新设计](../design/portable-update.md)。
