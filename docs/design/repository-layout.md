...
├── deployment/current/      当前使用的 EXE、运行库、配置、日志和 debug
...
部署目录保存用户当前使用的版本；候选构建留在源码 `.local/releases/`。两者之间只有明确的发布操作，不由普通构建自动覆盖。现有部署只迁移位置，EXE、运行库、配置与证据保持原内容。
...
└── .local/                 本机配置和生成物，不提交
    ...
    ├── build/              egg-info、setuptools/PyInstaller 中间文件与包装检查
    ├── releases/           便携目录与 ZIP
    ├── benchmarks/         离线基准结果
    ├── cache/              开发工具缓存
...
src 内职责与允许的依赖见[程序架构](architecture.md)。尚未实现的完整导航、岛屿目录和机制组合不预建空目录；功能达到需要独立维护的规模时再拆包。

## 配置、资源与运行数据

| 内容 | 唯一归属 | 维护规则 |
| --- | --- | --- |
| 默认配置 | `src/bd2_fishing/resources/default.ini` | 随 wheel/便携包发布，代码通过资源 API 读取，不再复制整份字符串 |
...
开发路径不随终端当前目录变化。普通 wheel 安装使用用户主目录的 `BD2_AutoFishing/` 保存配置，下设 logs 和 diagnostics；便携版为兼容现有使用继续在 EXE 旁保存配置、日志及 debug。自定义 OCR 相对资源仍从仓库或冻结资源目录解析，配置迁移不改变资源含义。

`.venv/` 含绝对路径，原位保留；以后重建环境再按标准命令生成。Python 缓存仍按解释器正常行为生成。setuptools 的 egg-info 统一位于 `.local/build/`，普通 wheel 中间文件位于 `.local/build/setuptools/`；setup.py 仅准备路径并调用构建后端，元数据和依赖仍由 pyproject 维护。
...
- 可导入的复用实现归 src，开发脚本只负责参数和调用，不能把业务逻辑藏进 scripts。
