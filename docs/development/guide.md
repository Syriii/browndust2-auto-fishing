...
桌面入口只加载页面、配置和记录服务，`bootstrap._run_task` 在工作线程首次执行时导入游戏会话。界面代码不要导入识别模块。ttk 底图中心使用平铺而非缩放；过小图块会大量调用原生绘制，不能仅靠不透明背景解决。图鉴首开创建控件，鱼图随可见区域解码，状态不变时不重建页面。检查使用 `smoke_catalogue.py --visible`，覆盖滚动到末尾和返回页面。
