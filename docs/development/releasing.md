...
构建读取 `src/bd2_fishing/resources/default.ini`，不读取个人 `.local/config.ini`。默认配置随包发布，首次运行便携版时在 EXE 旁生成 config.ini；已有用户配置优先。
...
Python 包安装和 wheel 构建的 egg-info 写入 `.local/build/`，中间文件写入 `.local/build/setuptools/`。setup.py 只配置这两个位置并自动创建目录；继续使用 pip/标准构建后端，不直接运行 setup.py 命令。
...
- `.local/releases/BD2_AutoFishing/`：完整便携目录。
- `.local/releases/BD2_AutoFishing-windows.zip`：分发 ZIP。
- `.local/build/pyinstaller/`、`.local/build/specs/`：中间文件与 spec。
...
```powershell
.\.local\releases\BD2_AutoFishing\BD2_AutoFishing.exe --preview
```
...
`.github/workflows/build.yml` 在 Release 发布或手动触发时运行，使用 Windows + Python 3.12.4，安装 requirements 中的锁定环境和本项目，运行离线回归与 scripts/build_release.py，上传 `.local/releases/BD2_AutoFishing-windows.zip`。发布 Release 时附加该 ZIP。

本地构建不自动提交、推送或发布。若使用自定义模型，文件也必须存在于远程检出内容中。

## 本机部署

本机工作区的用户版本位于外层 `deployment/current/`。仅明确要求部署时执行；普通构建只生成源码 `.local/releases/`。
...
本次目录整理只将旧部署迁入 deployment/current，未升级它。递归移动或删除前核对解析后的绝对路径，使用 PowerShell 原生文件命令。
