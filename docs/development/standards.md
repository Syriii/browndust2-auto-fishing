...
关闭先停止接收并封存账本，再有限等待观察线程。原生识别迟到返回时不能继续发布反馈或更新 OCR 读数，采集资源仍由原线程退出时释放。后台 OCR 与结算 OCR 串行，等待锁时响应取消，超过两秒未释放则结算保持未知。PNG/ZIP 编码和写盘继续在有界后台执行，三类证据写入器共用写入事务，失败时清理本次临时文件。消息固定、控制台/UI handler 和截图/输入驱动仍有开销，以上边界不是端到端时延保证。
